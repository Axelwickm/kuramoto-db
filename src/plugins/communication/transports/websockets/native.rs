#![cfg(all(feature = "ws_transport", feature = "ws_native", not(target_arch = "wasm32")))]
...
pub struct WsConn {
    writer: Arc<tokio::sync::Mutex<futures_util::stream::SplitSink<WebSocketStream<MaybeTlsStream<tokio::net::TcpStream>>, Message>>>,
...
impl TransportConn for WsConn {
    async fn send_bytes(&self, bytes: Vec<u8>) -> Result<(), TransportError> {
        ...
        w.send(Message::Binary(bytes)).await.map_err(|e| TransportError::Io(format!("ws send: {e}")))
    }
...
impl Connector for WsConnector {
    ...
    async fn dial(&self, addr: &Self::Addr) -> Result<Arc<dyn TransportConn>, TransportError> {
        let (ws, _resp) = connect_async(&addr.url).await.map_err(|e| TransportError::Io(format!("ws connect: {e}")))?;
        ...
        tokio::spawn(async move {
            while let Some(msg) = reader.next().await {
                match msg {
                    Ok(Message::Binary(b)) => { let _ = tx_bytes.send(b).await; }
                    Ok(Message::Text(s)) => { let _ = tx_bytes.send(s.into_bytes()).await; }
        ...
        Ok(Arc::new(WsConn { writer: Arc::new(tokio::sync::Mutex::new(writer)), rx_once: std::sync::Mutex::new(Some(rx_bytes)) }))
    }
}
...
async fn spawn_echo_server() -> Result<(SocketAddr, tokio::task::JoinHandle<()>), TransportError> {
    let listener = TcpListener::bind((std::net::Ipv4Addr::LOCALHOST, 0)).await.map_err(|e| TransportError::Io(format!("bind: {e}")))?;
    let addr = listener.local_addr().map_err(|e| TransportError::Io(format!("local_addr: {e}")))?;
            ...
            tokio::spawn(async move {
                if let Ok(mut ws) = tokio_tungstenite::accept_async(stream).await {
                    while let Some(m) = ws.next().await {
                        if let Ok(msg) = m { let _ = ws.send(msg).await; } else { break }
                    }
                }
            });
        }
...
mod tests {
    ...
    async fn websocket_roundtrip_against_echo_server() {
        ...
        let conn = WsConnector::new().dial(&WsAddr { peer: p, url }).await.unwrap();
