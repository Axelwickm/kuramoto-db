...
use crate::{
    ...
    plugins::{
        ...
        fnv1a_16,
        versioning::VERSIONING_AUX_ROLE,
    },
...
impl Handler for FetchProto {
    async fn on_request(&self, _peer: PeerId, body: &[u8]) -> Result<Vec<u8>, String> {
        let (req, _) =
            bincode::decode_from_slice::<ExportRequest, _>(body, bincode::config::standard())
        ...
        let scan_limit = if req.cursor.is_some() {
        ...
        let (out_rows, next, done) = match &req.mode {
            FetchMode::ByPkRange => {
                ...
                let rows = db
                    .range_raw_with_aux_by_pk_table(
                        &req.table,
                        aux_role,
                        scan_start,
                        &req.end,
                ...
                for (pk, data_opt, meta_raw) in rows.into_iter() {
                    ...
                    out_rows.push(RowEnvelope {
                        pk: pk.clone(),
                        tombstone,
                        data: data_opt.unwrap_or_default(),
                        meta: meta_raw,
                    });
                    next = Some(pk);
            ...
            FetchMode::ByIndexRange {
                ...
                let rows = db
                    .range_raw_with_aux_by_index_table(
                        index_table,
                        aux_role,
                        scan_start,
                        end_idx,
...
impl RemoteFetch {
    ...
    pub async fn fetch_from(
    ...
    ) -> Result<FetchStats, StorageError> {
        let db = self
            .db
            .get()
            .and_then(|w| w.upgrade())
            .ok_or(StorageError::Other("no db attached".into()))?;
        if req.limit == 0 {
            req.limit = self.default_limit;
        }
        ...
        loop {
            ...
            for row in &batch.rows {
                let applied = self.apply_row(&db, &batch.table, aux_role, row).await?;
                if applied {
                    stats.applied += 1;
                } else {
                    stats.skipped += 1;
            ...
            if batch.done || batch.next.is_none() {
                break;
            }
    ...
    async fn apply_row(
        ...
        let remote_meta: BlobMeta =
            bincode::decode_from_slice(&row.meta, bincode::config::standard())
        ...
        if let Some(lr) = local_meta_raw {
            if let Ok((lm, _)) =
                bincode::decode_from_slice::<BlobMeta, _>(&lr, bincode::config::standard())
            {
        ...
        if row.tombstone {
            db.delete_by_table_pk_with_origin(
                table,
                &row.pk,
                WriteOrigin::Plugin(fnv1a_16("remote_fetch")),
            )
            .await?;
        } else {
            db.put_by_table_bytes_with_origin(
                table,
                &row.data,
                WriteOrigin::Plugin(fnv1a_16("remote_fetch")),
            )
...
mod tests {
    ...
    static META: TableDefinition<'static, &'static [u8], Vec<u8>> =
        TableDefinition::new("f_test_meta");
    static AUX_TABLES: &[AuxTableSpec] = &[AuxTableSpec {
        role: VERSIONING_AUX_ROLE,
        table: &META,
    }];
    static IDX_NAME: TableDefinition<'static, &'static [u8], Vec<u8>> =
        TableDefinition::new("f_test_name_idx"); // UNIQUE
    static IDX_TAG: TableDefinition<'static, &'static [u8], Vec<u8>> =
        TableDefinition::new("f_test_tag_idx"); // NON-UNIQUE

    static INDEXES: &[IndexSpec<TEnt>] = &[
        IndexSpec {
            name: "name",
            key_fn: |e: &TEnt| e.name.as_bytes().to_vec(),
            table_def: &IDX_NAME,
            cardinality: IndexCardinality::Unique,
        },
        IndexSpec {
            name: "tag",
            key_fn: |e: &TEnt| e.tag.to_be_bytes().to_vec(),
            table_def: &IDX_TAG,
    ...
    async fn setup_pair(
        ...
        Arc<RemoteFetch>,
        Arc<Router>,
        crate::plugins::communication::transports::PeerId,
        Arc<KuramotoDb>,
        Arc<RemoteFetch>,
        Arc<Router>,
        crate::plugins::communication::transports::PeerId,
        ...
        let conn_a_to_b = a_conn
            .dial(&r.resolve(pid(2)).await.unwrap())
            .await
            .unwrap();
        let conn_b_to_a = b_conn
            .dial(&r.resolve(pid(1)).await.unwrap())
            .await
            .unwrap();
    ...
    async fn snapshot_import_basic() {
        ...
        let by_tag = db_b
            .get_by_index_all::<TEnt>(&IDX_TAG, &1u8.to_be_bytes())
            .await
            .unwrap();
    ...
    async fn error_on_unknown_table() {
        ...
        match err {
            StorageError::Other(msg) => assert!(msg.contains("router")),
    ...
    async fn index_unique_range_imports_subset() {
        ...
        let req = ExportRequest {
            ...
            mode: FetchMode::ByIndexRange {
                index_table: IDX_NAME.name().into(),
                start_idx: b"B".to_vec(),
                end_idx: b"Z".to_vec(),
            },
            aux_role: VERSIONING_AUX_ROLE.into(),
            limit: 0,
        ...
        assert_eq!(
            stats.applied, 2,
            "should import B and C by unique index range"
        );
    ...
    async fn index_nonunique_range_imports_all_matches() {
        ...
        let req = ExportRequest {
            ...
            mode: FetchMode::ByIndexRange {
                index_table: IDX_TAG.name().into(),
                start_idx: 1u8.to_be_bytes().to_vec(),
                end_idx: 2u8.to_be_bytes().to_vec(),
            },
            aux_role: VERSIONING_AUX_ROLE.into(),
            limit: 0,
            cursor: None,
