        ...
        for req in batch.iter_mut() {
            match req {
                WriteRequest::Put {
                    ...
                    let Some(versioning_table) =
                        db.resolve_aux_table_by_role(data_table.name(), VERSIONING_AUX_ROLE)
                    else {
                        continue;
                    };
...
impl VersioningPlugin {
    pub async fn get_meta<E: crate::storage_entity::StorageEntity>(
        ...
        let raw = db.get_aux_raw::<E>(VERSIONING_AUX_ROLE, key).await?;
    ...
    pub async fn get_with_meta<E: crate::storage_entity::StorageEntity>(
        ...
        let (e, raw) = db.get_with_aux_raw::<E>(VERSIONING_AUX_ROLE, key).await?;
...
mod tests {
    ...
    static TEST_AUX: &[AuxTableSpec] = &[AuxTableSpec {
    ...
    static TEST_VALUE_INDEX: TableDefinition<'static, &'static [u8], Vec<u8>> =
        TableDefinition::new("test_value_idx");
    static TEST_INDEXES: &[IndexSpec<TestEntity>] = &[
        ...
        IndexSpec {
            name: "value",
            key_fn: |e: &TestEntity| e.value.to_be_bytes().to_vec(),
    ...
    async fn overwrite_updates_meta() {
        ...
        let db = KuramotoDb::new(
            dir.path().join("v_over.redb").to_str().unwrap(),
            clock.clone(),
            vec![VersioningPlugin::new()],
        )
        .await;
        ...
        let m0 = VersioningPlugin::get_meta::<TestEntity>(&db, &e.id.to_be_bytes())
            .await
            .unwrap();
        ...
        e.value = 2;
        db.put(e.clone()).await.unwrap();
        let m1 = VersioningPlugin::get_meta::<TestEntity>(&db, &e.id.to_be_bytes())
            .await
            .unwrap();
    ...
    async fn delete_and_undelete() {
        ...
        let db = KuramotoDb::new(
            dir.path().join("v_del.redb").to_str().unwrap(),
            clock.clone(),
            vec![VersioningPlugin::new()],
        )
        .await;
        ...
        let md = VersioningPlugin::get_meta::<TestEntity>(&db, &e.id.to_be_bytes())
            .await
            .unwrap();
        ...
        clock.advance(5).await;
        db.put(e.clone()).await.unwrap();
        let mu = VersioningPlugin::get_meta::<TestEntity>(&db, &e.id.to_be_bytes())
            .await
            .unwrap();
    ...
    async fn stale_version_rejected() {
        ...
        let db = KuramotoDb::new(
            dir.path().join("v_stale.redb").to_str().unwrap(),
            clock.clone(),
            vec![VersioningPlugin::new()],
        )
        .await;
        ...
        let stale = VersioningPlugin::get_meta::<TestEntity>(&db, &e.id.to_be_bytes())
            .await
            .unwrap();
        ...
        let after = VersioningPlugin::get_meta::<TestEntity>(&db, &e.id.to_be_bytes())
            .await
            .unwrap();
    ...
    async fn range_with_meta_helpers() {
        ...
        let db = KuramotoDb::new(
            dir.path().join("v_range.redb").to_str().unwrap(),
            clock,
            vec![VersioningPlugin::new()],
        )
        .await;
        ...
        let got = VersioningPlugin::range_with_meta_by_pk::<TestEntity>(
            &db,
            &10u64.to_be_bytes(),
            &12u64.to_be_bytes(),
            None,
        ...
        let got2 = VersioningPlugin::range_with_meta_by_index::<TestEntity>(
            &db,
            &TEST_NAME_INDEX,
            b"N1",
            b"N3",
    ...
    async fn plugins_applied_in_order() {
        ...
        let db = KuramotoDb::new(
            dir.path().join("v_order.redb").to_str().unwrap(),
            clock.clone(),
            vec![
                VersioningPlugin::new(),
                Arc::new(Add100Plugin),
                Arc::new(DoublePlugin),
            ],
        )
        .await;
        db.create_table_and_indexes::<TestEntity>().unwrap();
        ...
        db.put(e.clone()).await.unwrap();
        let meta = VersioningPlugin::get_meta::<TestEntity>(&db, &1u64.to_be_bytes())
            .await
            .unwrap();
